...
function MilestoneCard({ m, index, isLast }: { m: typeof MILESTONES[0]; index: number; isLast: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center flex-shrink-0" style={{ width: 32 }}>
        ...
        {!isLast && (
          <motion.div
            ...
            style={{ background: `linear-gradient(to bottom, ${m.accent}50, rgba(255,255,255,0.04))`, minHeight: 40 }}
      ...
      {/* Card */}
      <motion.div
        className="flex-1 rounded-2xl p-5 mb-6 cursor-default select-none"
        style={{
...
export default function Journey() {
  ...
  const headerInView = useInView(headerRef, { once: true, margin: '0px 0px 300px 0px' })
  ...
  return (
    <section id="journey" aria-label="Professional journey and experience" className="relative py-28 overflow-hidden">
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-6 lg:px-8">
        ...
        <div>
          {MILESTONES.map((m, i) => (
            <MilestoneCard key={m.id} m={m} index={i} isLast={i === MILESTONES.length - 1} />
          ))}
        </div>
      </div>
    </section>
...
/*
 * ── HORIZONTAL CAROUSEL VERSION (preserved) ──────────────────────────────────
 *
 * To restore: replace the entire file content with the horizontal version below.
 *
  ...
  return (
    ...
    >
      <div className="flex items-center mb-5 gap-0">
        ...
        />
        <div className="flex-1 h-px ml-0" style={{ background: `linear-gradient(to right, ${m.accent}60, ${m.accent}10)` }} />
      </div>
      <motion.div
      ...
      >
        <div className="flex items-start gap-3 mb-3">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: `${m.accent}14` }}>
            <Icon size={16} style={{ color: m.accent }} />
          </div>
          <div>
            <div className="text-[10px] font-mono font-bold uppercase tracking-widest mb-0.5" style={{ color: m.accent }}>{m.year}</div>
            <h3 className="font-bold text-white leading-tight text-sm">{m.title}</h3>
            <p className="text-[11px] mt-0.5" style={{ color: `${m.accent}99` }}>{m.subtitle}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {m.tags.map((tag) => (
            <span key={tag} className="px-2 py-0.5 rounded text-[10px] font-medium"
              style={{ background: `${m.accent}10`, border: `1px solid ${m.accent}25`, color: `${m.accent}cc` }}>
              {tag}
            </span>
          ))}
        </div>
...
export default function Journey() {
  ...
  const progressPct = MILESTONES.length > 1 ? (activeIndex / (MILESTONES.length - 1)) * 100 : 0
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
        <div ref={headerRef}>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={headerInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
            ...
            <h2 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-4">
              <span className="text-white">Every great engineer</span><br />
              <span style={{ background: 'linear-gradient(135deg, #60a5fa 0%, #06b6d4 50%, #a855f7 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                has a journey.
              </span>
            </h2>
            <p className="text-white/35 text-sm mb-10 max-w-lg">From military service to AI systems — here's the path that shaped how I think about software.</p>
          </motion.div>
        </div>
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            {MILESTONES.map((_, i) => (
              <button key={i} onClick={() => scrollToIndex(i)} className="transition-all duration-300 rounded-full"
                style={{ width: i === activeIndex ? 20 : 6, height: 6, background: i === activeIndex ? MILESTONES[i].accent : 'rgba(255,255,255,0.12)' }} />
            ))}
          </div>
          <div className="flex items-center gap-2">
            <motion.button onClick={() => scrollTo('left')} className="w-9 h-9 rounded-xl flex items-center justify-center transition-colors"
              style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', opacity: canScrollLeft ? 1 : 0.3 }}
              whileHover={canScrollLeft ? { scale: 1.08, background: 'rgba(59,130,246,0.12)', borderColor: 'rgba(59,130,246,0.3)' } : {}}
              whileTap={canScrollLeft ? { scale: 0.95 } : {}} disabled={!canScrollLeft}>
              <ChevronLeft size={16} className="text-white/60" />
            </motion.button>
            <motion.button onClick={() => scrollTo('right')} className="w-9 h-9 rounded-xl flex items-center justify-center transition-colors"
              style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', opacity: canScrollRight ? 1 : 0.3 }}
              whileHover={canScrollRight ? { scale: 1.08, background: 'rgba(59,130,246,0.12)', borderColor: 'rgba(59,130,246,0.3)' } : {}}
              whileTap={canScrollRight ? { scale: 0.95 } : {}} disabled={!canScrollRight}>
              <ChevronRight size={16} className="text-white/60" />
            </motion.button>
          </div>
        </div>
        <div ref={scrollRef} className="flex gap-5 overflow-x-auto pb-4"
          style={{ scrollSnapType: 'x mandatory', scrollbarWidth: 'none', msOverflowStyle: 'none', cursor: 'grab' }}
          onMouseDown={(e) => {
            const el = scrollRef.current
            if (!el) return
        ...
        <div className="mt-6 h-px bg-white/[0.05] rounded-full overflow-hidden">
          <motion.div className="h-full rounded-full"
            style={{ background: 'linear-gradient(to right, #3b82f6, #06b6d4, #a855f7)', width: `${progressPct}%` }}
            animate={{ width: `${progressPct}%` }} transition={{ duration: 0.3, ease: 'easeOut' }} />
        </div>
        <motion.p className="text-center text-[11px] text-white/20 mt-3 font-mono"
